...
def task_transform(**context) -> None:
    ...
    clean_df = transform(pd.read_parquet(raw_path))
...
def task_load(**context) -> None:
    ...
    load(pd.read_parquet(clean_path), engine, settings.table_name)
...
def task_cleanup(**context) -> None:
    ...
    ds = context["ds"]
    for path in [TMP_DIR / f"raw_{ds}.parquet", TMP_DIR / f"clean_{ds}.parquet"]:
        if path.exists():
            os.remove(path)
...
with DAG(
    ...
    transform_task = PythonOperator(
        task_id="transform_task", python_callable=task_transform
    )
    load_task = PythonOperator(task_id="load_task", python_callable=task_load)
    cleanup_task = PythonOperator(
        task_id="cleanup_task", python_callable=task_cleanup, trigger_rule="all_done"
    )
