...
def create_table():
    con = psycopg2.connect(dbname="studentdb", user="postgres", password="password", host="localhost", port="5432")
    cur = con.cursor()
...
def insert_data():
    ...
    con = psycopg2.connect(dbname="studentdb", user="postgres", password="password", host="localhost", port="5432")
    cur = con.cursor()
    cur.execute("insert into students(name, address, age, number) values (%s, %s, %s, %s)",
